...
def model_get_user_by_username(username):
    conn = get_connection()
    cursor = conn.cursor()
...
def model_get_user_id_by_username(username):
    conn = get_connection()
    cursor = conn.cursor()
...
def model_get_steam_id_by_username(username):
    conn = get_connection()
    cursor = conn.cursor()
...
# Create a new user
def model_create_user(username, password, steam_id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
